...
(
	...
	ncm varchar(255),
	origem varchar(255),
	aliqfederal numeric(18,4),
	aliqestadual numeric(18,4),
	aliqmunicipal numeric(18,4),
	infadicional varchar(800)
...
INSERT INTO FORMAPGTO (ID, EXCLUIR, CRIADO, NOME) VALUES (1, 0, '18.09.2019 14:59:00', 'DINHEIRO');
...
BEGIN
  ...
  ELSE
  BEGIN
    tmp = GEN_ID(GEN_CONFIG_ID, 0);
    if (tmp < new.ID) then
      tmp = GEN_ID(GEN_CONFIG_ID, new.ID-tmp);
  END
END!!
SET TERM ; !!
...
CREATE GENERATOR GEN_NATUREZA_ID;

SET TERM !! ;
CREATE TRIGGER NATUREZA_BI FOR NATUREZA
ACTIVE BEFORE INSERT POSITION 0
AS
DECLARE VARIABLE tmp DECIMAL(18,0);
BEGIN
  IF (NEW.ID IS NULL) THEN
    NEW.ID = GEN_ID(GEN_NATUREZA_ID, 1);
  ELSE
  BEGIN
    tmp = GEN_ID(GEN_NATUREZA_ID, 0);
    if (tmp < new.ID) then
      tmp = GEN_ID(GEN_NATUREZA_ID, new.ID-tmp);
  END
END!!
SET TERM ; !!

CREATE GENERATOR GEN_NOTA_ID;

SET TERM !! ;
CREATE TRIGGER NOTA_BI FOR NOTA
ACTIVE BEFORE INSERT POSITION 0
AS
DECLARE VARIABLE tmp DECIMAL(18,0);
BEGIN
  IF (NEW.ID IS NULL) THEN
    NEW.ID = GEN_ID(GEN_NOTA_ID, 1);
  ELSE
  BEGIN
    tmp = GEN_ID(GEN_NOTA_ID, 0);
    if (tmp < new.ID) then
      tmp = GEN_ID(GEN_NOTA_ID, new.ID-tmp);
  END
END!!
SET TERM ; !!
