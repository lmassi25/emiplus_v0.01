...
namespace Emiplus.Controller
{
    public class Fiscal
    {
        ...
        private PessoaContato _destinatarioContato;
        private PessoaEndereco _destinatarioEndereco;

        private Model.Pessoa _emitente;
        private PessoaContato _emitenteContato;
        private PessoaEndereco _emitenteEndereco;
        ...
        private readonly int _servidorNFCE = 2;

        private int _servidorNFE = 2;
        private int _servidorNFSE = 2;

        private Model.Pessoa _transportadora;
        private PessoaContato _transportadoraContato;
        private PessoaEndereco _transportadoraEndereco;
        ...
        private readonly string loteNFSE = "1";
        ...
        private readonly string TECNOSPEED_CIDADEUF = "SAOJOSEDORIOPRETOSP";
        ...
        /// </summary>
        private async Task updateUltNfeAsync()
        {
        ...
        private void Start(int Pedido = 0, string tipo = "NFe", int Nota = 0)
        {
            ...
            switch (tipo)
            {
                ...
                case "CFe":
                    ...
                    layoutCFE = !string.IsNullOrEmpty(IniFile.Read("Layout", "SAT")) ? IniFile.Read("Layout", "SAT") : "0.08";
            ...
            TECNOSPEED_SERVIDOR = _servidorNFE == 1 ? "https://managersaas.tecnospeed.com.br:8081/" : "https://managersaashom.tecnospeed.com.br:7071/";
            ...
            _emitenteEndereco = new PessoaEndereco();

            //_emitenteContato = new Model.PessoaContato().FindById(_id_empresa).First<Model.PessoaContato>();
            _emitenteContato = new PessoaContato();

            _emitente.RG = Validation.CleanStringForFiscal(Settings.Default.empresa_inscricao_estadual)
                .Replace(" ", "");
        ...
        public string Emitir(int Pedido, string tipo = "NFe", int Nota = 0, bool imposto = true)
        {
            ...
            try
            {
                ...
                if (imposto)
                {
                    ...
                    foreach (var item in itens.Get()) new Imposto().SetImposto(item.ID, 0, tipo);
                }
                ...
                switch (tipo)
                {
                    ...
                    case "NFe":
                        ...
                        if (IniFile.Read("NFe", "APP") == "Uninfe")
                        {
                            ...
                            var arqPath = _path_enviada + "\\Cancela" + Pedido + ".xml";

                            if (string.IsNullOrEmpty(_nota.protocolodeuso)) uninfe_consulta();

                            var writer = new XmlTextWriter(arqPath, null);
                            ...
                            writer.WriteElementString("idLote",
                                "0000000000" + DateTime.Now.Minute.ToString("00") + DateTime.Now.Second.ToString("00") +
                                "5");
                            ...
                            writer.WriteElementString("CNPJ",
                                Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", ""));
                            writer.WriteElementString("chNFe", _nota.ChaveDeAcesso);
                            writer.WriteElementString("dhEvento",
                                Validation.DateNowToSql() + "T" + DateTime.Now.ToString("HH:mm:ss") +
                    ...
                    case "CFe":
                        ...
                        _nota = new Model.Nota().FindByIdPedidoUltReg(Pedido, "Autorizada")
                            .FirstOrDefault<Model.Nota>();
                        ...
                        var rdn = new Random();
                        _msg = Sat.StringFromNativeUtf8(Sat.CancelarUltimaVenda(rdn.Next(999999), GetCodAtivacao(),
                            _nota.ChaveDeAcesso, arq.OuterXml));

                        if (_msg.Contains("Cupom cancelado com sucesso"))
                        {
                            var txt = new StreamWriter(_path_enviada + "\\" + _pedido.Id + "Canc.txt", false,
                                Encoding.UTF8);
                            txt.Write(_msg);
                            txt.Close();

                            if (!Directory.Exists(_path_autorizada + "\\" + DateTime.Now.Year +
                            ...
                            try
                            {
                                ...
                                ChaveDeAcesso = oXML.SelectSingleNode("/CFeCanc/infCFe").Attributes.GetNamedItem("Id")
                                    .Value;
                                ChaveDeAcessoCanc = oXML.SelectSingleNode("/CFeCanc/infCFe").Attributes
                                    .GetNamedItem("chCanc").Value;

                                nr_Nota = oXML.SelectSingleNode("/CFeCanc/infCFe/ide").ChildNodes[4].InnerText;
                                assinatura_qrcode = oXML.SelectSingleNode("/CFeCanc/infCFe/ide").ChildNodes[10]
                                    .InnerText;

                                var doc = XDocument.Parse(Base64ToString(Sep_Delimitador('|', 6, _msg)));
                                doc.Save(_path_autorizada + "\\" + DateTime.Now.Year +
                                         DateTime.Now.Month.ToString("00") + "\\CFeCanc" +
                                         ChaveDeAcesso.Replace("CFe", "") + ".xml");
                                ...
                                doc.Save(
                                    _path_autorizada + "\\bkp\\CFeCanc" + ChaveDeAcesso.Replace("CFe", "") + ".xml");
                                //------------------------nota salva
            ...
            switch (tipo)
            {
                case "CFe":
                    ...
                    switch (_nota.Status)
                    {
                        case "Autorizada":
                        {
                            ...
                            printer.Append(_emitenteEndereco.Rua + ", " + _emitenteEndereco.Nr + " - " +
                                           _emitenteEndereco.Bairro);
                            ...
                            pagamentos = new Model.Titulo().Query().Where("titulo.id_pedido", Pedido)
                            ...
                            var totais = new Model.PedidoItem().Query()
                                .LeftJoin("item", "pedido_item.item", "item.id")
                                .SelectRaw(
                                    "SUM(pedido_item.federal) as federal, SUM(pedido_item.estadual) as estadual, SUM(pedido_item.municipal) as municipal")
                                .Where("pedido_item.pedido", Pedido)
                                .Where("pedido_item.excluir", 0)
                                .Where("pedido_item.tipo", "Produtos")
                                .Get();
                            ...
                            foreach (var data in totais)
                                printer.CondensedMode(AddSpaces("Valor aproximado dos tributos deste cupom ",
                                    Validation.FormatDecimalPrice(data.FEDERAL + data.ESTADUAL + data.MUNICIPAL, true)));
                            ...
                            var cfeid = _nota.ChaveDeAcesso.Replace("CFe", "");
                            ...
                            if (!string.IsNullOrEmpty(_pedido.cfe_cpf))
                                qrcode = _nota.ChaveDeAcesso.Replace("CFe", "") + "|" + _nota.Criado.Year.ToString("0000") +
                                         _nota.Criado.Month.ToString("00") + _nota.Criado.Day.ToString("00") +
                                         _nota.Criado.Hour.ToString("00") + _nota.Criado.Minute.ToString("00") +
                                         _nota.Criado.Second.ToString("00") + "|" + total_qrcode + "|" + _pedido.cfe_cpf +
                                         "|" + _nota.assinatura_qrcode;
                            else
                                qrcode = _nota.ChaveDeAcesso.Replace("CFe", "") + "|" + _nota.Criado.Year.ToString("0000") +
                                         _nota.Criado.Month.ToString("00") + _nota.Criado.Day.ToString("00") +
                                         _nota.Criado.Hour.ToString("00") + _nota.Criado.Minute.ToString("00") +
                                         _nota.Criado.Second.ToString("00") + "|" + total_qrcode + "|" + "" + "|" +
                                         _nota.assinatura_qrcode;
                        ...
                        case "Cancelado":
                        {
                            ...
                            var cfeid = _nota.ChaveDeAcesso.Replace("CFe", "");
                            cfeid = cfeid.Substring(0, 4) + " " + cfeid.Substring(4, 4) + " " + cfeid.Substring(8, 4) +
                                    " " + cfeid.Substring(12, 4) + " " + cfeid.Substring(17, 4) + " " +
                                    cfeid.Substring(21, 4) + " " + cfeid.Substring(25, 4) + " " + cfeid.Substring(29, 4) +
                                    " " + cfeid.Substring(33, 4) + " " + cfeid.Substring(37, 4) + " " +
                                    cfeid.Substring(40, 4);
                ...
                case "NFe":
                    ...
                    if (pdfNFE.Contains(".pdf"))
                    {
                        ...
                        if (!File.Exists(_path_autorizada + "\\NFe.pdf")) return "Arquivo não encontrado!";

                        //BrowserNfe.Render = pdf;
                        //browser.ShowDialog();

                        Process.Start(_path_autorizada + "\\NFe.pdf");
        ...
        private void CriarXML(int Pedido, string tipo, int modelo = 0)
        {
            ...
            if (tipo != "CFe")
            {
                ...
                {
                    _destinatarioEndereco = new PessoaEndereco().FindById(_pedido.id_useraddress)
                        .FirstOrDefault<PessoaEndereco>();
                }
                else if (_pedido.Cliente > 1)
                {
                    _destinatarioEndereco = new PessoaEndereco().FindByIdUser(_pedido.Cliente)
                        .FirstOrDefault<PessoaEndereco>();
                    ...
                    {
                        _msg =
                            "Endereço do destinátário não encontrado. É necessário informar o endereço do destinatário para emitir. Clique no botão 'Ver Detalhes' para alterar.";
                        return;
                    }
                }
                ...
                {
                    var checkNatureza = new Model.Natureza().Query().Where("NOME", "VENDA")
                        .FirstOrDefault<Model.Natureza>();
                    if (checkNatureza == null)
                    {
                        var nat = new Model.Natureza
                        {
                            Id = 0, 
                            Nome = "VENDA"
                        };
            ...
            switch (modelo)
            {
                case 1:
                    ...
                    xmlCanc.WriteElementString("dest",
                        !string.IsNullOrEmpty(_pedido.cfe_cpf)
                            ? Validation.CleanStringForFiscal(_pedido.cfe_cpf).Replace(".", "").Replace(" ", "")
                            : "");
                ...
                default:
                    ...
                    if (tipo == "NFSe")
                    {
                        ...
                        var txt = new StreamWriter(strFilePath, true, Encoding.UTF8);
                        ...
                        txt.Write("CPFCNPJRemetente=" +
                                  Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                                  Environment.NewLine);
                        txt.Write("InscricaoMunicipalRemetente=" +
                                  Validation.CleanStringForFiscal(Settings.Default.empresa_inscricao_municipal) +
                                  Environment.NewLine);
                        txt.Write("ValorTotalServicos=" + Validation.FormatPriceWithDot(_pedido.Servicos) +
                                  Environment.NewLine);
                        txt.Write("ValorTotalDeducoes=0.00" + Environment.NewLine);
                        txt.Write("ValorTotalBaseCalculo=" + Validation.FormatPriceWithDot(_pedido.Servicos) +
                                  Environment.NewLine);
                        ...
                        txt.Write("DataEmissao=" + Validation.ConvertDateToSql(_pedido.Emissao) + "T" +
                                  DateTime.Now.Hour.ToString("00") + ":" + DateTime.Now.Minute.ToString("00") + ":" +
                                  DateTime.Now.Second.ToString("00") + Environment.NewLine);
                        txt.Write("Competencia=" + Validation.ConvertDateToSql(_pedido.Emissao) + Environment.NewLine);

                        txt.Write("CpfCnpjPrestador=" +
                                  Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                                  Environment.NewLine);
                        txt.Write("InscricaoMunicipalPrestador=" +
                                  Validation.CleanStringForFiscal(Settings.Default.empresa_inscricao_municipal) +
                                  Environment.NewLine);
                        txt.Write("RazaoSocialPrestador=" + Validation.CleanStringForFiscal(_emitente.Nome) +
                                  Environment.NewLine);
                        txt.Write("InscricaoEstadualPrestador=" + Validation.CleanStringForFiscal(_emitente.RG) +
                                  Environment.NewLine);
                        txt.Write("InscricaoMunicipalPrestador=" +
                                  Validation.CleanStringForFiscal(Settings.Default.empresa_inscricao_municipal) +
                                  Environment.NewLine);
                        txt.Write("TipoLogradouroPrestador=Rua" + Environment.NewLine);
                        txt.Write("EnderecoPrestador=" + Validation.CleanStringForFiscal(_emitenteEndereco.Rua) +
                                  Environment.NewLine);
                        txt.Write("NumeroPrestador=" + Validation.CleanStringForFiscal(_emitenteEndereco.Nr) +
                                  Environment.NewLine);
                        ...
                        txt.Write("BairroPrestador=" + Validation.CleanStringForFiscal(_emitenteEndereco.Bairro) +
                                  Environment.NewLine);
                        txt.Write("CodigoCidadePrestador=" + Validation.CleanStringForFiscal(_emitenteEndereco.IBGE) +
                                  Environment.NewLine);
                        txt.Write("DescricaoCidadePrestador=" +
                                  Validation.CleanStringForFiscal(_emitenteEndereco.Cidade) + Environment.NewLine);
                        txt.Write("TelefonePrestador=" + Validation.CleanStringForFiscal(_emitenteContato.Telefone) +
                                  Environment.NewLine);
                        txt.Write("EmailPrestador=" + Validation.CleanStringForFiscal(_emitenteContato.Email) +
                                  Environment.NewLine);
                        txt.Write("CepPrestador=" + Validation.CleanStringForFiscal(_emitenteEndereco.Cep) +
                                  Environment.NewLine);
                        txt.Write(Environment.NewLine);
                        ...
                        txt.Write("CodigoItemListaServico=" + Settings.Default.empresa_codigoitemnfse +
                                  Environment.NewLine);
                        txt.Write("CodigoTributacaoMunicipio=" + Settings.Default.empresa_codigotributacaonfse +
                                  Environment.NewLine);
                        ...
                        txt.Write("MunicipioIncidencia=" + Validation.CleanStringForFiscal(_emitenteEndereco.IBGE) +
                                  Environment.NewLine);
                        txt.Write("CodigoCidadePrestacao=" + Validation.CleanStringForFiscal(_emitenteEndereco.IBGE) +
                                  Environment.NewLine);
                        txt.Write("DescricaoCidadePrestacao=" +
                                  Validation.CleanStringForFiscal(_emitenteEndereco.Cidade) + Environment.NewLine);
                        txt.Write(Environment.NewLine);

                        txt.Write("CpfCnpjTomador=" +
                                  Validation.CleanStringForFiscal(_destinatario.CPF.Replace(".", "").Replace(" ", "")) +
                                  Environment.NewLine);
                        txt.Write("RazaoSocialTomador=" + Validation.CleanStringForFiscal(_destinatario.Nome) +
                                  Environment.NewLine);
                        ...
                        if (_destinatarioEndereco != null)
                        {
                            ...
                            txt.Write("EnderecoTomador=" + Validation.CleanStringForFiscal(_destinatarioEndereco.Rua) +
                                      Environment.NewLine);
                            ...
                            txt.Write("BairroTomador=" + Validation.CleanStringForFiscal(_destinatarioEndereco.Bairro) +
                                      Environment.NewLine);
                            txt.Write("CodigoCidadeTomador=" + _destinatarioEndereco.IBGE + Environment.NewLine);
                            txt.Write("DescricaoCidadeTomador=" +
                                      Validation.CleanStringForFiscal(_destinatarioEndereco.Cidade) +
                                      Environment.NewLine);
                            txt.Write("UfTomador=" + _destinatarioEndereco.Estado + Environment.NewLine);
                            txt.Write("CepTomador=" + Validation.CleanStringForFiscal(_destinatarioEndereco.Cep) +
                                      Environment.NewLine);
                        ...
                        if (_destinatarioContato != null)
                        {
                            ...
                            txt.Write("TelefoneTomador=" +
                                      Validation.CleanStringForFiscal(_destinatarioContato.Telefone) +
                                      Environment.NewLine);
                            txt.Write("EmailTomador=" + Validation.CleanStringForFiscal(_destinatarioContato.Email) +
                                      Environment.NewLine);
                        }
                        ...
                        txt.Write("ValorServicos=" + Validation.FormatPriceWithDot(_pedido.Servicos) +
                                  Environment.NewLine);
                        txt.Write(
                            "BaseCalculo=" + Validation.FormatPriceWithDot(_pedido.Servicos) + Environment.NewLine);
                        ...
                        txt.Write("ValorLiquidoNfse=" + Validation.FormatPriceWithDot(_pedido.Servicos) +
                                  Environment.NewLine);
                    ...
                    else
                    {
                        ...
                        var xml = new XmlTextWriter(strFilePath, Encoding.UTF8) {Formatting = Formatting.Indented};
                        ...
                        switch (tipo)
                        {
                            case "NFCe":
                                ...
                                chvAcesso = codUF(_emitenteEndereco.Estado) + _pedido.Emissao.ToString("yy") +
                                            _pedido.Emissao.ToString("MM") +
                                            Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") + "65" + serie +
                                            nNF + "1" + cNF;
                            ...
                            default:
                                ...
                                chvAcesso = (codUF(_emitenteEndereco.Estado) + _pedido.Emissao.ToString("yy") +
                                             _pedido.Emissao.ToString("MM") +
                                             Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") + "55" +
                                             serie + nNF + "1" + cNF).Replace(" ", "");
                                cDV = CalculoCDV(chvAcesso);
                                chvAcesso = chvAcesso + "" + cDV;
            ...
            switch (tipo)
            {
                ...
                case "NFe":
                    ...
                    else
                    {
                        ...
                        {
                            if (_msg.Contains(
                                "já existe no banco de dados. E não pode ser alterada pois ela está REGISTRADA."))
                            {
                                _msg = RequestResolve();
                            }
                            else if (_msg.Contains("Autorizado o uso") || _msg.Contains(
                                "já existe no banco de dados. E não pode ser alterada pois ela está AUTORIZADA.")
                            )
                            {
                ...
                case "NFCe":
                    ...
                    while (!string.IsNullOrEmpty(_msg) && done == false)
                    {
                        ...
                        if (_msg.Contains("Autorizado o uso") || _msg.Contains(
                            "já existe no banco de dados. E não pode ser alterada pois ela está AUTORIZADA."))
                        {
                            _msg = "Autorizado o uso da NF-e";
                            _nota.Tipo = tipo;
                ...
                case "CFe":
                    ...
                    var txt = new StreamWriter(
                        _path_enviada + "\\" + _pedido.Id + "_" + Validation.RandomSecurity() + ".txt", false,
                        Encoding.UTF8);
                    txt.Write(_msg);
                    txt.Close();
                    ...
                    if (string.IsNullOrEmpty(_msg))
                    {
                        ...
                        _msg =
                            "SAT não retornou mensagem. É possível que este cupom esteja autorizado ou não. Entre em contato com o suporte técnico!";
        ...
        private void ValidarXML(int Pedido, string tipo = "NFe")
        {
            //Boolean done = false;
            var arq = new XmlDocument();
            var arqPath = _path_enviada + "\\" + _pedido.Id + ".xml";
        ...
        {
            var _notaCCe = new Model.Nota();
            _notaCCe = _notaCCe.Query().Where("status", "Transmitindo...").Where("id_pedido", Pedido)
                .Where("excluir", 0).FirstOrDefault<Model.Nota>();
            ...
            {
                SeqEvento = new Nota().GetSeqCCe(Pedido, Nota);
                _notaCCe.Serie = SeqEvento;
            }

            var arqPath = _path_enviada + "\\CCe" + _pedido.Id + ".tx2";

            if (File.Exists(arqPath)) File.Delete(arqPath);

            #region TX2

            var tx2 = new StreamWriter(arqPath, true, Encoding.UTF8);

            tx2.WriteLine("Documento=CCE");
            tx2.WriteLine("ChaveNota=" + _nota.ChaveDeAcesso);
            tx2.WriteLine("dhEvento=" + Validation.DateNowToSql() + "T" + DateTime.Now.Hour.ToString("00") + ":" +
            ...
            tx2.WriteLine("Lote=" + "0000000000" + DateTime.Now.Minute.ToString("00") +
                          DateTime.Now.Second.ToString("00") + "5");
        ...
        public string ImprimirCCe(int Pedido, int Nota)
        {
            ...
            Process.Start(_path_autorizada + "\\CCe.pdf");
        ...
        {
            var _inutiliza = new Model.Nota();
            _inutiliza = _inutiliza.Query().Where("status", "Transmitindo...").Where("tipo", "Inutiliza")
                .Where("excluir", 0).FirstOrDefault<Model.Nota>();
            ...
            _msg = RequestSendInutiliza("&Ano=" + _inutiliza.Criado.ToString("yy") + "&Serie=" + _inutiliza.Serie +
                                        "&NFIni=" + _inutiliza.nr_Nota + "&NFFin=" + _inutiliza.assinatura_qrcode +
                                        "&Justificativa=" + _inutiliza.correcao);
        ...
        private void SetIde(XmlTextWriter xml, int Pedido, string tipo)
        {
            ...
            else
            {
                ...
                string horaSaida = string.IsNullOrEmpty(_pedido.HoraSaida) ? DateTime.Now.ToString("HH:mm:ss") : _pedido.HoraSaida + ":00" ;
                
                xml.WriteElementString("serie",
                    string.IsNullOrEmpty(Settings.Default.empresa_nfe_serienfe)
                        ? "0"
                        : Settings.Default.empresa_nfe_serienfe);
                xml.WriteElementString("nNF", getLastNFeNr().ToString());
                xml.WriteElementString("dhEmi",
                    Validation.ConvertDateToSql(_pedido.Emissao) + "T" + DateTime.Now.Hour.ToString("00") + ":" +
                    DateTime.Now.Minute.ToString("00") + ":" + DateTime.Now.Second.ToString("00") +
                    DateTime.Now.ToString("zzz"));

                if (tipo != "NFCe")
                    xml.WriteElementString("dhSaiEnt",
                        Validation.ConvertDateToSql(_pedido.Emissao) + "T" + horaSaida + DateTime.Now.ToString("zzz"));
        ...
        private void SetNFref(XmlTextWriter xml, int Pedido, string tipo)
        {
            ...
            var notas = new Model.Nota().Query().Select("chavedeacesso").Where("Nota.id_pedido", _id_nota)
                .Where("Nota.excluir", 0).Where("Nota.tipo", "Documento").OrderBy("Nota.id").Get();
        ...
        private void SetDest(XmlTextWriter xml, int Pedido, string tipo)
        {
            if (tipo != "NFCe")
            {
                ...
                if (tipo == "CFe")
                {
                    if (!string.IsNullOrEmpty(_pedido.cfe_cpf))
                    {
                        xml.WriteElementString(_pedido.cfe_cpf.Length == 11 ? "CPF" : "CNPJ",
                            Validation.CleanStringForFiscal(_pedido.cfe_cpf).Replace(".", "").Replace(" ", ""));
                    }
                }
                else
                {
                    ...
                    {
                        if (_destinatario.CPF == null || Validation.CleanStringForFiscal(_destinatario.CPF)
                            .Replace(".", "").Replace(" ", "") == "")
                    ...
                    xml.WriteElementString("xNome",
                        Settings.Default.empresa_nfe_servidornfe.ToString() == "2"
                            ? "NF-E EMITIDA EM AMBIENTE DE HOMOLOGACAO - SEM VALOR FISCAL"
                            : Validation.CleanStringForFiscal(_destinatario.Nome));
        ...
        private void SetDet(XmlTextWriter xml, int Pedido, string tipo, int n, int id)
        {
            ...
            if (tipo == "CFe")
            {
                ...
                xml.WriteElementString("vOutro", Validation.FormatPriceWithDot(_pedidoItem.Frete));
            ...
            if (totalTrib > 0)
                xml.WriteElementString("infAdProd",
                    "VALOR APROXIMADO DOS TRIBUTOS: FEDERAL: R$ " + Validation.FormatPriceWithDot(_pedidoItem.Federal) +
                    " - ESTADUAL: R$ " + Validation.FormatPriceWithDot(_pedidoItem.Estadual) + " - MUNICIPAL: R$ " +
                    Validation.FormatPriceWithDot(_pedidoItem.Municipal) + " - FONTE: IBPT");
            ...
            if (tipo != "CFe")
            {
                ...
                foreach (var data in query)
                    xml.WriteElementString("vTotTrib",
                        Validation.FormatPriceWithDot(Validation.ConvertToDouble(data.FEDERAL) +
                                                      Validation.ConvertToDouble(data.ESTADUAL) +
                                                      Validation.ConvertToDouble(data.MUNICIPAL)));
        ...
        private void SetTransp(XmlTextWriter xml, int Pedido, string tipo)
        {
            ...
            xml.WriteElementString("modFrete", tipo != "NFCe" ? _pedido.TipoFrete.ToString() : "9");
            ...
            if (_pedido.Id_Transportadora > 0)
            {
                ...
                _transportadoraEndereco = new PessoaEndereco().FindByIdUser(_pedido.Id_Transportadora)
                    .FirstOrDefault<PessoaEndereco>();

                xml.WriteStartElement("transporta");

                xml.WriteElementString(_transportadora.Pessoatipo == "Física" ? "CPF" : "CNPJ",
                    Validation.CleanStringForFiscal(_transportadora.CPF).Replace(".", "").Replace(" ", ""));
                ...
                if (_transportadoraEndereco != null)
                {
                    if (!string.IsNullOrEmpty(_transportadoraEndereco.Rua))
                    {
                        ...
                        var xEnder = Validation.CleanStringForFiscal(_transportadoraEndereco.Rua) + " - " +
                                     _transportadoraEndereco.Nr + " - " +
                                     Validation.CleanStringForFiscal(_transportadoraEndereco.Bairro);
                        if (xEnder.Length > 60) xEnder = xEnder.Substring(0, 60);
        ...
        private void SetPag(XmlTextWriter xml, int Pedido, string tipo)
        {
            if (tipo != "CFe")
            {
                ...
                pagamentos = new Model.Titulo().Query().Select("Total").Where("titulo.id_pedido", Pedido).Where("titulo.excluir", 0).Get();
                foreach (var data in pagamentos) total = total + Validation.ConvertToDouble(data.TOTAL);

                //SE O PAGAMENTO LANÇADO NÃO FOI IGUAL AO TOTAL DO PEDIDO NÃO INFORMA PAGAMENTO
                if (_pedido.Total != total)
                {
                ...
                pagamentos = new Model.Titulo().Query().Where("titulo.id_pedido", Pedido).Where("titulo.excluir", 0)
                    .OrderBy("titulo.vencimento").Get();
                if (pagamentos.Any())
                {
                    countFat = 1;
                ...
                if (pagamentos.Any())
                {
                    ...
                    foreach (var data in pagamentos)
                    {
                        SetDetPag(xml, data.ID_FORMAPGTO.ToString("00"), Validation.FormatPriceWithDot(data.TOTAL), 2,
                            count.ToString("000"), Validation.ConvertDateToSql(data.VENCIMENTO));
                        count++;
                    }
                }
                ...
                if (countFat == 1) xml.WriteEndElement(); //cobr
            }
            ...
            {
                pagamentos = new Model.Titulo().Query().Select("titulo.id_formapgto as id_formapgto")
                    .SelectRaw("SUM(titulo.total) as total").Where("titulo.id_pedido", Pedido)
                    .Where("titulo.excluir", 0).GroupBy("titulo.id_formapgto").Get();
        ...
        private void SetDetPag(XmlTextWriter xml, string formaPgto = "90", string valor = "0.00", int tag = 0,
            string n = "", string venc = "")
        {
            switch (tag)
            {
                case 1:
        ...
        private void SetInfAdic(XmlTextWriter xml, int Pedido, string tipo)
        {
            ...
            if (!string.IsNullOrEmpty(_pedido.info_fisco))
            {
                ...
                xml.WriteElementString("infAdFisco",
                    Validation.OneSpaceString(Validation.CleanStringForFiscal(_pedido.info_fisco)));
                tag = true;
            }

            if (!string.IsNullOrEmpty(_pedido.info_contribuinte))
            {
                if (tag == false) xml.WriteStartElement("infAdic");
                xml.WriteElementString("infCpl",
                    Validation.OneSpaceString(Validation.CleanStringForFiscal(_pedido.info_contribuinte)));
                tag = true;
            }

            if (tag) xml.WriteEndElement(); //infAdic
        }

        private string CalculoCDV(string chave)
        {
            var chaveAcesso = chave;

            var chaveInvertida = ReverseString(chave);
            int[] t = {2, 3, 4, 5, 6, 7, 8, 9};
            var somatorio = 0;
            var posicaoParaCalculo = 0;
        ...
        {
            var arrChar = prStringEntrada.ToCharArray();
            Array.Reverse(arrChar);
            var invertida = new string(arrChar);
        ...
        public string RequestConsultCpf()
        {
            ...
            var retorno = request(requestData, "nfe", "conscad", "GET");
        ...
        {
            requestData = "encode=true&cnpj=" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                          "&grupo=" + TECNOSPEED_GRUPO + tx2;
        ...
        {
            requestData = "encode=true&cnpj=" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                          "&grupo=" + TECNOSPEED_GRUPO + "&arquivo=" + tx2;
        ...
        private string RequestSend(string xml, string documento = "NFe")
        {
            if (documento == "NFSe")
                requestData = "encode=true&cnpj=" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                              "&nomecidade=" + TECNOSPEED_CIDADEUF + "&grupo=" + TECNOSPEED_GRUPO + "&arquivo=" +
                              HttpUtility.UrlEncode(xml);
            else
                requestData = "encode=true&cnpj=" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                              "&grupo=" + TECNOSPEED_GRUPO + "&arquivo=" + HttpUtility.UrlEncode(xml);
        ...
        {
            requestData = "encode=true&cnpj=" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                          "&grupo=" + TECNOSPEED_GRUPO + "&ChaveNota=" + chvAcesso + "&Justificativa=" + justificativa;
        ...
        {
            requestData = "encode=true&cnpj=" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                          "&grupo=" + TECNOSPEED_GRUPO + "&Campos=situacao&Filtro=" + chvAcesso;
            return request(requestData, documento, "consulta", "GET");
        }

        private string RequestConsultAutorizada(string documento = "NFe")
        {
            requestData = "encode=true&cnpj=" + _emitente.CPF + "&grupo=" + TECNOSPEED_GRUPO +
                          "&Campos=chave,dtemissao,dtautorizacao,nrecibo,situacao,nnf,nlote,idintegracao,ambiente&Limite=1&Filtro=" +
                          chvAcesso;
            return request(requestData, documento, "consulta", "GET");
        }
        ...
        {
            requestData = "encode=true&cnpj=" + _emitente.CPF + "&grupo=" + TECNOSPEED_GRUPO + "&ChaveNota=" +
                          chvAcesso;
        ...
        private string RequestPrint(string chavedeacesso, string documento = "NFe")
        {
            ...
            {
                case "CFe":
                    requestData = "encode=true&cnpj=" +
                                  Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") + "&grupo=" +
                                  TECNOSPEED_GRUPO + "&ChaveNota=" + chavedeacesso + "&Url=1";
                    break;

                case "CCe":
                    requestData = "encode=true&cnpj=" +
                                  Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") + "&grupo=" +
                                  TECNOSPEED_GRUPO + "&ChaveNota=" + chavedeacesso + "&Url=1&Documento=CCe";
                    break;
        ...
        private string RequestEmail(string email, string documento = "NFe")
        {
            var conteudoemail = "<![CDATA[<html> " +
                                ...
                                "<p> O código chave de sua Nota Fiscal Eletrônica é " + chvAcesso +
            ...
            requestData = "encode=true&cnpj=" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                          "&grupo=" + TECNOSPEED_GRUPO + "&EmailDestinatario=" + email + "&ChaveNota=" + chvAcesso +
                          "&Assunto=Nota Fiscal Eletrônica&Texto=" + conteudoemail + "&AnexaPDF=1&ConteudoHTML=1";
        ...
        private string RequestImport(string xml, string documento = "CFe")
        {
            ...
            return request(requestData, documento == "CFe" ? "cfesat" : "NFe", "importa", "POST");
        }

        private string request(string _requestData, string _documento, string _route, string _method)
        {
            return getMessage(_method == "POST" ? postRequest(_requestData, _documento, _route) : getRequest(_requestData, _documento, _route));
        }
        ...
        {
            var url = TECNOSPEED_SERVIDOR + "ManagerAPIWeb/" + get_documento.ToLower() + "/" + get_route + "?" +
                      get_requestData;
            var _request = new WebClient();
            var credentials = new UTF8Encoding().GetBytes(TECNOSPEED_USUARIO + ":" + TECNOSPEED_SENHA);
        ...
        private string postRequest(string post_requestData, string post_documento, string post_route)
        {
            ...
            try
            {
                ...
                var request = WebRequest.Create(TECNOSPEED_SERVIDOR + "ManagerAPIWeb/" + post_documento.ToLower() +
                                                "/" + post_route);
                ...
                var byteArray = Encoding.UTF8.GetBytes(post_requestData);
        ...
        {
            return IniFile.Read("Servidor", "SAT") == "Homologacao" ? "12345678" : IniFile.Read("Codigo_Ativacao", "SAT");
        }

        private static string Sep_Delimitador(char sep, int posicao, string dados)
        {
            var ret_dados = dados.Split(sep);
        ...
        public string Logs(int tipo = 0)
        {
            ...
            var ret = StringFromNativeUtf8(tipo == 0 ? Sat.ExtrairLogs(rdn.Next(999999), GetCodAtivacao()) : Sat.ConsultarStatusOperacional(rdn.Next(999999), GetCodAtivacao()));
            ...
            if (!string.IsNullOrEmpty(ret))
            {
                if (tipo == 0)
                {
                    ...
                    foreach (var item in ret.Split(caracter.ToCharArray(), StringSplitOptions.RemoveEmptyEntries))
        ...
        // Set Barcode height
        private static readonly byte[] SetBarcodeHeight = {0x1D, 0x68, 0x25};

        // Set Barcode width
        private static readonly byte[] SetBarcodeWidth = {0x1D, 0x77, 0x03};

        // Begin barcode printing
        private static readonly byte[] EAN13BarCodeStart = {0x1D, 0x6B, 67, 13};

        public static string BarcodeString(string barcode)
        {
            var s = Encoding.ASCII.GetString(SetBarcodeHeight);
            s += Encoding.ASCII.GetString(SetBarcodeWidth);
            s += $"{Encoding.ASCII.GetString(EAN13BarCodeStart)}{barcode}";
        ...
        private void uninfe_envia(string tipo = "envia", string arqPath = "")
        {
            ...
            string _pathUninfe =
                    @"C:\Emiplus\Unimake\UniNFe\" + Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", ""),
            ...
            var pastaRetorno = new DirectoryInfo(_pathRetorno);
            ...
            while (!encontrado)
            {
                var findfiles = pastaRetorno.GetFiles("*");
                foreach (var fileinfo in findfiles)
                    switch (tipo)
                    {
                        default:
                            ...
                            else if (File.Exists(_pathRetorno + "\\" + _nota.ChaveDeAcesso + "-nfe.err"))
                            {
                                _msg = File.ReadAllText(_pathRetorno + "\\" + _nota.ChaveDeAcesso + "-nfe.err");
                                encontrado = true;
                            }
                            else if (fileinfo.Name.IndexOf("-rec.err") > 0)
                            {
                                _msg = File.ReadAllText(_pathRetorno + "\\" + fileinfo.Name);
                                encontrado = true;
                            }
                            else if (File.Exists(_pathRetorno + "\\canc" + _nota.ChaveDeAcesso + "-eve.err"))
                            {
                                _msg = File.ReadAllText(_pathRetorno + "\\canc" + _nota.ChaveDeAcesso + "-eve.err");
                                encontrado = true;
                            }
                            else if (File.Exists(_pathRetorno + "\\canc" + _nota.ChaveDeAcesso + "-eve.xml"))
                            {
                                encontrado = true;
                            }
            ...
            var files = pastaRetorno.GetFiles("*");
            foreach (var fileinfo in files)
                if (fileinfo.Name.IndexOf("-pro-rec.xml") > 0)
                {
                    ...
                    var xnList = oXML.GetElementsByTagName("infProt");
                ...
                else if (fileinfo.Name.IndexOf("-sit.xml") > 0)
                {
                    ...
                    {
                        var xnList = oXML.GetElementsByTagName("infCanc");
                ...
                else if (fileinfo.Name.IndexOf("-eve.xml") > 0)
                {
                    ...
                    var oXML = new XmlDocument();
                    oXML.Load(_pathRetorno + "\\" + fileinfo.Name);
                    var xnList = oXML.GetElementsByTagName("infEvento");
        ...
        {
            var _pathUninfe = @"C:\Emiplus\Unimake\UniNFe\" +
                              Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "");
            ...
            var writer = new XmlTextWriter(_pathEnvio + "\\" + _nota.ChaveDeAcesso + "-ped-sit.xml", null);
        ...
        private void uninfe_xmlAutorizado(string chavedeacesso)
        {
            var _pathUninfe = @"C:\Emiplus\Unimake\UniNFe\" +
                              Validation.CleanStringForFiscal(_emitente.CPF).Replace(".", "") +
                              "\\Enviado\\Autorizados\\20" + chavedeacesso.Substring(2, 4);
            var mensagem = "";
            ...
            if (File.Exists(_pathUninfe + "\\" + chavedeacesso + "-procNFe.xml"))
            {
                ...
                var xnList = oXML.GetElementsByTagName("ide");

                for (var i = 0; i < xnList.Count; i++)
                {
                    _nota = new Model.Nota().Query().Where("CHAVEDEACESSO", chavedeacesso).Where("excluir", 0)
                        .FirstOrDefault<Model.Nota>();
